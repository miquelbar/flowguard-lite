...
func (s *APIServer) handlePostSettings(w http.ResponseWriter, r *http.Request) {
	...
	if payload.DDoSThresholdPPS < 1 || payload.SYNFloodThresholdPPS < 1 || payload.UDPFloodThresholdPPS < 1 || payload.ICMPFloodThresholdPPS < 1 {
...
func (s *APIServer) handleTestNotificationRule(w http.ResponseWriter, r *http.Request) {
	...
	mockIP := "192.168.1.99"
	if rule.Scope == "ip" && rule.Target != "" {
		mockIP = rule.Target
	} else if rule.Scope == "subnet" && rule.Target != "" {
