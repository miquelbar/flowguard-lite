...
func (r *Repository) SaveNotificationRule(ctx context.Context, rule *NotificationRule) error {
	...
	if err != nil {
		return fmt.Errorf("failed to start notification rule transaction: %w", err)
	}
	defer tx.Rollback()
	...
	if err := saveSQLiteAuditLog(ctx, tx, "save_notification_rule", auditDetails); err != nil {
		return err
	}
...
func (r *Repository) DeleteNotificationRule(ctx context.Context, id int64) error {
	...
	tx, err := r.metaDB.BeginTx(ctx, nil)
	if err != nil {
		return fmt.Errorf("failed to start notification rule delete transaction: %w", err)
	}
	defer tx.Rollback()
	...
	if err := saveSQLiteAuditLog(ctx, tx, "delete_notification_rule", auditDetails); err != nil {
		return err
	}
