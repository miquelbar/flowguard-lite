...
func TestSQLiteSaveNotificationRuleRollsBackWhenAuditFails(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "flowguard-test-notification-acid-*")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
	defer os.RemoveAll(tmpDir)
	...
	if _, err := repo.metaDB.ExecContext(ctx, `
		CREATE TRIGGER block_notification_rule_save_audit
		BEFORE INSERT ON audit_logs
		WHEN NEW.action = 'save_notification_rule'
		BEGIN
			SELECT RAISE(ABORT, 'notification audit blocked');
		END;
	`); err != nil {
...
func TestSQLiteDeleteNotificationRuleRollsBackWhenAuditFails(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "flowguard-test-notification-acid-*")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
	...
	if _, err := repo.metaDB.ExecContext(ctx, `
		CREATE TRIGGER block_notification_rule_delete_audit
		BEFORE INSERT ON audit_logs
		WHEN NEW.action = 'delete_notification_rule'
		BEGIN
			SELECT RAISE(ABORT, 'notification audit blocked');
		END;
	`); err != nil {
