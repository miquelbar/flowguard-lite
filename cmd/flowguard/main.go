...
func main() {
	...
	// 4. Initialize API HTTP server
	server := api.NewAPIServer(cfg, log)

	// 5. Run HTTP server in a separate goroutine so we can trap signals concurrently
	...
	// 6. Setup signal trapping for graceful shutdown
	signalChan := make(chan os.Signal, 1)
	signal.Notify(signalChan, syscall.SIGINT, syscall.SIGTERM, syscall.SIGHUP)

	// 7. Wait for an termination signal or server start failure
	select {
	...
	case sig := <-signalChan:
		...
		// Create a timeout context for the shutdown operations (e.g., 5 seconds)
