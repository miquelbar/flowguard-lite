import { state, setSelectedNotificationRuleId, setNotificationRuleEditorDirty } from '../../app/state.js';
...
} from './notificationRulePreview.js';
...
export function renderNotificationRules() {
    ...
    if (state.notificationRulesError) {
        tblNotificationRules.innerHTML = renderTableMessage(7, "error", `Failed to load rules: ${state.notificationRulesError}`);
        return;
    }

    if (!state.notificationRulesData || state.notificationRulesData.length === 0) {
        tblNotificationRules.innerHTML = renderTableMessage(7, "empty", "No notification rules configured yet.");
        return;
    }
        ...
        return `
            <tr data-id="${r.id}" class="${isSelected ? 'selected' : ''}" style="cursor: pointer;">
                ...
                <td class="text-center">
                    <button class="btn-secondary btn-select-rule" data-id="${r.id}" aria-label="Select notification rule ${escapeHtml(r.name)}">Select</button>
                </td>
            </tr>
        `;
    }).join('');
...
export function renderNotificationLogs() {
    ...
    if (state.notificationLogsError) {
        tblNotificationLogs.innerHTML = renderTableMessage(6, "error", `Failed to load logs: ${state.notificationLogsError}`);
        return;
    }

    if (!state.notificationLogsData || state.notificationLogsData.length === 0) {
        tblNotificationLogs.innerHTML = renderTableMessage(6, "empty", "No notification audit logs found.");
        return;
    }
...
export function selectNotificationRuleId(id) {
    if (state.selectedNotificationRuleId !== id && state.notificationRuleEditorDirty) {
        if (!confirm("You have unsaved changes in this notification rule. Discard them?")) {
            // Restore visual selection in nav to match current state
            renderNotificationRules();
            return;
        }
    }
    setSelectedNotificationRuleId(id);
    setNotificationRuleEditorDirty(false);
...
function selectNotificationRule(r) {
    ...
    updateNotificationRulePreview();
    focusFirstVisible(["#notification-rule-name"]);
}

export function startAddNotificationRule() {
    if (state.notificationRuleEditorDirty) {
        if (!confirm("You have unsaved changes in this notification rule. Discard them?")) {
            return;
        }
    }
    setSelectedNotificationRuleId("new");
    setNotificationRuleEditorDirty(false);
...
export function resetNotificationRuleDetails() {
    setSelectedNotificationRuleId(null);
    setNotificationRuleEditorDirty(false);
    const notificationDetailsEmpty = document.getElementById("notification-details-empty");
    const notificationDetailsContent = document.getElementById("notification-details-content");
...
export function bindNotificationsEvents(onReload) {
    ...
    if (selectRuleScope) {
        selectRuleScope.addEventListener("change", () => {
            setNotificationRuleEditorDirty(true);
            updateNotificationTargetFieldVisibility();
            updateNotificationRulePreview();
        });
    }
    ...
    bindSplitPaneClose(
        ["btn-cancel-notification-rule", "btn-close-notification-details"],
        "#/notifications",
        () => {
            resetNotificationRuleDetails();
            renderNotificationRules();
        },
        ["#btn-add-notification-rule"]
    );
}
